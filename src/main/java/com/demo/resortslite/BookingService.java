...
public class BookingService {
    ...
    // Cached credentials (retrieved from AWS Secrets Manager)
     ...
     * This enables secure credential rotation without code changes.
     */
    private void loadDatabaseCredentials() {
        try {
            GetSecretValueRequest request = GetSecretValueRequest.builder()
                    .secretId(dbSecretsName)
                    .build();
    ...
    /**
     * Creates a booking using parameterized queries to prevent SQL injection.
     ...
     */
    public Map<String, String> getAuthenticationCredentials(String username) {
        Map<String, String> credentials = new HashMap<>();
        try {
            GetSecretValueRequest request = GetSecretValueRequest.builder()
                    .secretId(authSecretsName + "/" + username)
                    .build();
            GetSecretValueResponse response = secretsManagerClient.getSecretValue(request);
            String secretString = response.secretString();
            ...
            credentials.put("passwordHash", secretJson.get("passwordHash").asText());
            credentials.put("role", secretJson.get("role").asText());
        } catch (Exception e) {
            // Return empty credentials if not found
            credentials.put("error", "Credentials not found for user: " + username);
        }
        return credentials;
    }
